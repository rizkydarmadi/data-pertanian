...
st.write(
    "Pemanasan global adalah kejadian meningkatnya temperatur rata-rata atmosfer,laut dan daratan Bumi. Permasalahan teresebut menyebabkan meningkatnya suhu global diperkirakan akan menyebabkan perubahan-perubahan yang lain seperti naiknya permukaan air laut, meningkatnya intensitas fenomena cuaca yang ekstrim, Akibat-akibat dari meningkatnya suhu global diantaranya adalah terpengaruhnya hasil pertanian."
)
st.write(
    ":signal_strength:  [Coba Prediksi](#prediksi-suhu-rata-rata-4-tahun-ke-depan)"
)
...
mean_temp = pd.DataFrame({"year": year, "mean_temp": mean_temperature})
scaler = preprocessing.MinMaxScaler()
x_scaled = scaler.fit_transform(mean_temp["mean_temp"].values.reshape(-1, 1))
x = x_scaled.flatten()
mean_temp = pd.DataFrame({"year": year, "mean_temp": x})
st.write("## Grafik rata-rata suhu sejak tahun 1900 - 2021 di Indonesia")
st.write(
    "Data berikut ini adalah hasil rata rata suhu tahunan yang telah di normalisasi dengan skala min max yang diambil dari tahun 1900 - 2021 dan dipat dari situs world bank"
)
st.line_chart(mean_temp, x="year", y="mean_temp")
mean_temp = pd.DataFrame({"year": year, "mean_temp": mean_temperature})
st.write(
    "berdasarkan data diatas dapat dilihat bahwa ada kenaikan suhu yang menerus dari 1970 hingga saat ini"
)
...
d = st.date_input(
    "Masukan tahun yang akan di prediksi suhunya", datetime.date(2022, 1, 1)
)

prediksi_tahun = model_2.predict([[d.year]])

st.write(
    f"#### Hasil Prediksi Suhu Rata-Rata Pada Tahun {d.year} adalah {round(prediksi_tahun[0],2)} celcius"
)
...
st.write(
    "Berikut ini adalah data produksi hasil 25 jenis hasil pertanian di indonesia dari tahun ke tahun. 25 jenis pertanian diantaranya adalah Bawang Merah, Bawang Putih, Bawang Daun, Kentang, Kubis, Kembang Kol, Petsai/Sawi, Wortel, Lobak, Kacang Merah, Kacang Panjang, Cabai Besar, Cabai Rawit, Tomat, Terung, Buncis, Ketimun, Labu Siam, Kangkung, Bayam, Melinjo, Paprika, Jamur, Petai dan Jengkol."
)
st.bar_chart(top_anual_value, y="total", x="date", height=600)
st.write(
    "Berdasarkan grafik diatas total hasil produksi pada tahun 2014 - 2019 mengalami kenaikan cukup tinggi"
)
...
df1 = pd.DataFrame({"value": mean_temperature})
kacang_tanah = pd.read_csv("dataset/kacang_tanah.csv")

df3 = pd.DataFrame({"x": df1["value"].values, "y": kacang_tanah["total"]})
df3 = df3.head(21)
...
st.write(
    "Berikut adalah korelasi hubungan antara suhu rata rata dengan produksi kacang merah di indonesia"
)
fig = plt.figure()
...
st.pyplot(fig)
st.write(
    "berdasarkan grafik diatas terdapat korelasi yang kuat antara suhu rata-rata dengan produksi kacang merah di indonesia."
)
st.write(
    "### Prediksi hasil prediksi hasil produksi kacang merah berdasarkan suhu rata-rata"
)
df001 = pd.DataFrame(
    {
...
st.write(
    f"#### Hasil prediksi kacang merah pada suhu {number} adalah {round(prediksi_k_merah[0],2)} Ton"
)
...
st.write(
    "Berikut adalah korelasi hubungan antara suhu rata rata dengan produksi apel malang di indonesia"
)
...
st.pyplot(fig)
st.write(
    "berdasarkan grafik diatas terdapat korelasi yang kuat antara suhu rata-rata dengan produksi apel malang di indonesia."
)
st.write("### Prediksi suhu rata-rata 4 tahun ke depan")
df001 = pd.DataFrame(
    {
...
st.write(
    f"#### Hasil prediksi apel malang pada suhu {number} adalah {round(prediksi_apel_malang[0],2)} ton"
)
...
#! disclaimer
st.write(
    "Seluruh data yang ada di dalam penelitian ini bersumber dari [Badan pusat statistik](https://www.bps.go.id), [World bank](https://climatedata.worldbank.org/ClimateAPIWeb/rest/v2/crunew/cru-ts4.06-timeseries/tas/annual/timeseries/1901-2021/country/IDN) dan penelitian terdahulu."
)
